...
def scrape_rimi_lv_pages():
    ...
    try:
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as pool:
            for products in pool.map(scrape_rimi_lv_page, pages):
...
def scrape_rimi_lv():
    ...
    for cat_id, cat_name in scrape_rimi_lv_categories():
        if not db.session.get(Category, cat_id):
            db.session.add(Category(id=cat_id, name=cat_name, store="rimi_lv"))
    ...
    for item in products:
        ...
        if not db.session.get(Category, category_id):
            ...
            db.session.add(Category(id=category_id, name="Unknown", store="rimi_lv"))
